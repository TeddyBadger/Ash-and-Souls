### **Overview**

These incantations harness the power of The Formless One, allowing followers to conjure and manipulate blood.

---
### **Bloodflame Blade**

Applies bloodflame to the weapon in your other hand, or can be cast onto another persons weapon at the cost of 2 further ticks. This changes the damage type dealt to fire, allows the attacker to add "Resolve" to the damage they deal and cause an extra 15 bleed build up on hit.

|     **Info**     | **Value** |
| :--------------: | :-------: |
|      Damage      | + Resolve |
|   Aether Cost    |    13     |
|     Priority     |    17     |
| Attunement Slots |     1     |
| Projectile Speed |    N/A    |
|  Ticks to Cast   |  1 or 3   |
|   Action Type    |   Half    |
|  Stat Build-Up   |    N/A    |
|   Damage Type    |   Fire    |
|   Poise Damage   |    N/A    |

---
### **Bloodboon**

Thrust your arm upwards into a seal of The Formless One and pull down and forwards to spray blood outwards. This causes damage on hit, bleed build up and leaves a pool of burning blood on the floor in front of you. This will cover a 1m x 2m rectangle. For every tick someone remains in this area, they recieve bleed build up. This area remains for "Resolve" amount of ticks.

|     **Info**     |         **Value**         |
| :--------------: | :-----------------------: |
|      Damage      |       1d8 + Resolve       |
|   Aether Cost    |            15             |
|     Priority     |            15             |
| Attunement Slots |             1             |
| Projectile Speed |         3m / Tick         |
|  Ticks to Cast   |             1             |
|   Action Type    |           Full            |
|  Stat Build-Up   | 25 + 10 per tick<br>Bleed |
|   Damage Type    |           Fire            |
|   Poise Damage   |            N/A            |

---
### **Bloodflame Talons**

You slash across the air forming three slash marks that do damage and bleed build. 2 ticks after casting, it explodes dealing further damage and bleed build up.

|     **Info**     |         **Value**         |
| :--------------: | :-----------------------: |
|      Damage      | 1d8 + Resolve<br>then 1d4 |
|   Aether Cost    |            19             |
|     Priority     |            15             |
| Attunement Slots |             1             |
| Projectile Speed |         3m / Tick         |
|  Ticks to Cast   |             1             |
|   Action Type    |           Full            |
|  Stat Build-Up   |       25 + 10 Bleed       |
|   Damage Type    |           Fire            |
|   Poise Damage   |            N/A            |

---
### **Swarm of Bloodflies**

You conjure a small swarm of bloodflies that home towards your target, tracking them and slowly causing bleed build up per tick. This swarm lasts for 4 ticks.

|     **Info**     |  **Value**  |
| :--------------: | :---------: |
|      Damage      |     N/A     |
|   Aether Cost    |     15      |
|     Priority     |     11      |
| Attunement Slots |      1      |
| Projectile Speed |  1m / Tick  |
|  Ticks to Cast   |      1      |
|   Action Type    |    Full     |
|  Stat Build-Up   | 15 per tick |
|   Damage Type    |     N/A     |
|   Poise Damage   |     N/A     |

---
### **Formless One's Blades**

Lunge forward 2m and slash three times in front of you. Each slash does damage and bleed build up. It takes a tick to lunge and another tick to slash three times.

|     **Info**     |         **Value**          |
| :--------------: | :------------------------: |
|      Damage      | 1d4 + Resolve<br>per slash |
|   Aether Cost    |             21             |
|     Priority     |             15             |
| Attunement Slots |             1              |
| Projectile Speed |            N/A             |
|  Ticks to Cast   |             1              |
|   Action Type    |            Full            |
|  Stat Build-Up   |   20 Bleed<br>per slash    |
|   Damage Type    |            Fire            |
|   Poise Damage   |            N/A             |
